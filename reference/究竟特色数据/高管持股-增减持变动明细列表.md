...
| 名称 | 类型 | 必填 | 默认值 | 说明 |
|:---|:---|:---|:---|:---|
| `interfaceId` | `str` | 是 | "DRAGON_TIGER_MANAGER_HOLDING_CHANGE_LIST_E8127FAB" | 接口ID |
| `pageIndex` | `int` | 是 | - | 当前页从0开始 |
| `pageSize` | `int` | 是 | - | 每页显示记录数 |
| `changeType` | `int` | 否 | - | 变动类型（增持：1，减持：-1, 0：全部 默认） |
| `startDate` | `str` | 否 | - | 开始时间 |
| `endDate` | `str` | 否 | - | 结束时间 |
...
data = api.query_kjj_data(
    interfaceId="DRAGON_TIGER_MANAGER_HOLDING_CHANGE_LIST_E8127FAB",
