...
| `fundDate` | `str` | 否 | 成立日期 |
| `issueDate` | `str` | 否 | 发行日期 |
| `issueShare` | `float` | 否 | 发行份额 (亿) |
| `managementFee` | `float` | 否 | 管理费 |
| `trusteeFee` | `float` | 否 | 托管费 |
| `duration` | `str` | 否 | 存续期 |
| `faceValue` | `float` | 否 | 面值 |
| `minAmount` | `float` | 否 | 起点金额 (万元) |
| `expectedReturn` | `float` | 否 | 预期收益率 |
