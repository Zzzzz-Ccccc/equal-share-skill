# 主营业务数据分析历史数据

> 分类: 财务数据 | 目录: `财务数据` | 索引见 `SKILL.md`


### 描述

主营业务数据分析历史数据

### 请求参数

| 名称 | 类型 | 必填 | 默认值 | 说明 |
|:---|:---|:---|:---|:---|
| `interfaceId` | `str` | 是 | "FINANCE_INDEX_BUSINESS_ANALYSIS_HISTORY_DATA_A113AD31" | 接口ID |
| `stockCodeList` | `list[str]` | 是 | - | 股票代码列表 示例：['000001', '000002'] |
| `reportDate` | `str` | 否 | - | 报告期,如 2025-12-31，默认为最新一期 |
| `filterType` | `int` | 否 | `1` | 筛选方式 1-按年报 2-按中报 示例：1 |

### 返回参数

ResultData.data 为 List，元素为主营业务分析历史数据

| 名称 | 类型 | 是否必返回 | 说明 |
|:---|:---|:---|:---|
| `dateStr` | `str` | 否 | 报告期 |
| `list` | `list[EastStockBusinessAnalysisVo]` | 否 | 主营业务分析列表 |

**`list`** 数组元素结构：

| 名称 | 类型 | 是否必返回 | 说明 |
|:---|:---|:---|:---|
| `stockCode` | `str` | 否 | 股票代码 |
| `stockName` | `str` | 否 | 股票名称 |
| `reportTime` | `str` | 否 | 报告期 |
| `classType` | `str` | 否 | 分类类型 |
| `rank` | `int` | 否 | 排名 |
| `value` | `str` | 否 | 值 |
| `change` | `str` | 否 | 变化 |
| `changeRate` | `str` | 否 | 变化率 |

### 调用示例

```python
from equal_data import KjjApi 
api = KjjApi('bear your token') 
# 调用接口
data = api.query_kjj_data(
    interfaceId="FINANCE_INDEX_BUSINESS_ANALYSIS_HISTORY_DATA_A113AD31",
    stockCodeList=None,  # list[str]  # 默认: 
    reportDate=None,  # str  # 默认: 
    filterType=None,  # int  # 默认: 1
)
```


